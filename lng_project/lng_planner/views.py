...
def export_excel(request, simulation_id):
    ...
    for plant in plants:
        ...
        # ── Refineries ───────────────────────────────────────────────────────
        for refinery in simulation.refineries.filter(plant=plant):
            row = [f'    {refinery.name}']
            for day in daily_data:
                pd = day['plant_data'].get(plant.id, {})
                val = next((s['amount'] for s in pd.get('supplies', [])
                            if s['type'] == 'refinery' and s['name'] == refinery.name), None)
                row.append(val if val else '')
            ws.append(row)
            _style_row(ws[ws.max_row], C_REFINERY, fc=FC_PURPLE)
            ...
            try:
                ...
                if 'suppliers' in data:
                    for s in data['suppliers']:
                        ...
                        supplier_ranges = s.get('date_ranges')
                        if not supplier_ranges and {'from_date', 'to_date', 'daily_supply'} <= s.keys():
                            supplier_ranges = [{
                                'from_date': s['from_date'],
                                'to_date': s['to_date'],
                                'daily_supply': s['daily_supply'],
                            }]
                        for date_range in supplier_ranges or []:
                            SupplierDate.objects.create(
                                supplier=supplier,
                ...
                if 'customers' in data:
                    for c in data['customers']:
                        ...
                        customer_ranges = c.get('date_ranges')
                        if not customer_ranges and {'from_date', 'to_date', 'daily_demand'} <= c.keys():
                            customer_ranges = [{
                                'from_date': c['from_date'],
                                'to_date': c['to_date'],
                                'daily_demand': c['daily_demand'],
                            }]
                        for date_range in customer_ranges or []:
                            CustomerDate.objects.create(
                                customer=customer,
                                from_date=date_range['from_date'],
                                to_date=date_range['to_date'],
                ...
                if 'refineries' in data:
                    for r in data['refineries']:
                        ...
                        refinery_ranges = r.get('date_ranges')
                        if not refinery_ranges and {'from_date', 'to_date', 'daily_refinery_supply'} <= r.keys():
                            refinery_ranges = [{
                                'from_date': r['from_date'],
                                'to_date': r['to_date'],
                                'daily_refinery_supply': r['daily_refinery_supply'],
                            }]
                        for date_range in refinery_ranges or []:
                            RefineryDate.objects.create(
                                refinery=refinery,
                                from_date=date_range['from_date'],
                                to_date=date_range['to_date'],
                                daily_refinery_supply=date_range['daily_refinery_supply']
                            )
    ...
    while current_date <= end_date:
        ...
        for plant_id, inventory in plant_inventories.items():
            ...
            for supplier in simulation.suppliers.filter(plant_id=plant_id):
                ...
                for supplier_date in supplier.date_ranges.all():
                    if supplier_date.from_date <= current_date <= supplier_date.to_date:
                        supplier_amount += float(supplier_date.daily_supply)
                if supplier_amount > 0:
                    supplies.append({'type': 'supplier', 'name': supplier.name, 'amount': supplier_amount})
                    plant_supply += supplier_amount

            for refinery in simulation.refineries.filter(plant_id=plant_id):
                # Refinery behaves like a supplier with range-based supply.
                refinery_amount = 0.0
                for refinery_date in refinery.date_ranges.all():
                    if refinery_date.from_date <= current_date <= refinery_date.to_date:
                        refinery_amount += float(refinery_date.daily_refinery_supply)
                if refinery_amount > 0:
                    supplies.append({'type': 'refinery', 'name': refinery.name, 'amount': refinery_amount})
                    plant_supply += refinery_amount
            ...
            for customer in simulation.customers.filter(plant_id=plant_id):
                # Collect matching customer ranges on this date so each range can contribute its own demand.
                active_ranges = []
                for customer_date in customer.date_ranges.all():
                    if customer_date.from_date <= current_date <= customer_date.to_date:
                        active_ranges.append(customer_date)
                ...
                if active_ranges:
                    ...
                    for customer_date in active_ranges:
...
def mock_sap_api(request):
    ...
    return JsonResponse({
        ...
        "suppliers": [
            {"name": "Supplier A", "plant_name": "Dahej", "date_ranges": [{"from_date": "2026-01-01", "to_date": "2026-12-31", "daily_supply": 1}]},
            {"name": "Supplier B", "plant_name": "Ennore", "date_ranges": [{"from_date": "2026-01-01", "to_date": "2026-06-30", "daily_supply": 3}]}
        ],
        "cargos": [
        ...
        "customers": [
            {"name": "Customer 1",  "plant_name": "Dahej", "date_ranges": [{"from_date": "2026-01-01", "to_date": "2026-12-31", "daily_demand": 5}]},
            {"name": "Customer 2",  "plant_name": "Ennore", "date_ranges": [{"from_date": "2026-01-01", "to_date": "2026-08-31", "daily_demand": 10}]},
            {"name": "Customer 10", "plant_name": "Ennore", "date_ranges": [{"from_date": "2026-01-01", "to_date": "2026-08-31", "daily_demand": 10}]}
        ],
        "refineries": [
            {"name": "Refinery A", "plant_name": "Dahej", "date_ranges": [{"from_date": "2026-01-01", "to_date": "2026-12-31", "daily_refinery_supply": 2}]}
        ]
    })
